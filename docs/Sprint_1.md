...
```bash
npm create vite@latest my-reading-extension -- --template react-ts
cd my-reading-extension
npm install
```
...
const Popup: React.FC = () => {
  ...
  useEffect(() => {
    const initSettings = async () => {
      ...
      const savedFontSize = await getStorage<number>(StorageKeys.FONT_SIZE, 'sync');
