...
export class ListExtractor {
  ...
  public createEnhancedList(listInfo: ListInfo): HTMLElement {
    ...
    // 检查是否是任务列表
    const isTaskList = this.isTaskList(listInfo);
    if (isTaskList) {
      list.classList.add('enhanced-task-list');
    }
    ...
    listInfo.items.forEach(itemInfo => {
      ...
      // 处理任务列表项
      if (isTaskList) {
        const isChecked = this.isCheckedTask(itemInfo.text);
        if (isChecked) {
          item.classList.add('checked');
        }

        // 移除任务标记
        itemInfo.text = this.removeTaskMarker(itemInfo.text);
      }
      ...
      // 处理文本中的链接和其他内容
      textSpan.innerHTML = this.processListItemText(itemInfo.text);
   ...
   * 检查是否是任务列表
   */
  private isTaskList(listInfo: ListInfo): boolean {
    if (listInfo.type !== 'unordered') return false;

    // 检查是否至少有一个列表项包含任务标记
    return listInfo.items.some(item => {
      return /^\s*\[[ xX]\]\s/.test(item.text);
    });
  }
   ...
   * 检查任务是否已完成
   */
  private isCheckedTask(text: string): boolean {
   ...
   * 移除任务标记
   */
  private removeTaskMarker(text: string): string {
   ...
   * 处理列表项文本
   */
  private processListItemText(text: string): string {
    // 处理链接
    ...
    // 处理内联代码
    text = text.replace(/`([^`]+)`/g, '<code class="enhanced-inline-code">$1</code>');
